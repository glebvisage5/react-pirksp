...
router.put("/projects/:projectId", requireAuth, async (req, res, next) => {
...
// ─── Specs ────────────────────────────────────────────────────────────────────
router.get("/:id/specs", requireAuth, async (req, res, next) => {
  try { res.json(await svc.listSpecs(req.params["id"]!)); } catch (e) { next(e); }
});

router.post("/:id/specs", requireAuth, async (req, res, next) => {
...
router.get("/specs/:specId", requireAuth, async (req, res, next) => {
  try { res.json(await svc.getSpec(req.params["specId"]!)); } catch (e) { next(e); }
});

router.put("/specs/:specId", requireAuth, async (req, res, next) => {
...
router.delete("/specs/:specId", requireAuth, async (req, res, next) => {
  try { await svc.deleteSpec(req.params["specId"]!, req.user!.userId); res.json({ ok: true }); } catch (e) { next(e); }
});

router.get("/specs/:specId/versions", requireAuth, async (req, res, next) => {
...
router.post("/:id/files", requireAuth, upload.single("file"), async (req: Request & { file?: Express.Multer.File }, res, next) => {
...
router.post("/:id/roles", requireAuth, async (req, res, next) => {
...
router.put("/roles/:roleId", requireAuth, async (req, res, next) => {
  const d = validate(roleSchema, req.body, next); if (!d) return;
